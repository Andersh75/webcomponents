class TableBaseCE extends CustomElement3 {
	...
	extendView(that, model) {
		this.view.renderCellsCapitalize = function(obj) {
			console.log('renderCellsCapitalize');
			console.log('that.srdispatch');
			console.log(that.srdispatch);
			console.log(obj);

			let srRemoteAndLocals = h.str.stringToArrayUsingSplitter('@', that.srdispatch); //makes an array of [remote, local...] listener
			let testAr = srRemoteAndLocals[0].slice(1, -1);
			console.log(testAr);
			let testAr2 = h.str.stringToArrayUsingSplitter(',', testAr);
			
			let srChannelAndSubject = h.str.stringToArrayUsingSplitter(':', testAr2[0]); //makes an array of [remote, local...] listener
			let srChannel = srChannelAndSubject[0];
			let srSubject = srChannelAndSubject[1];
			let srLocal = srRemoteAndLocals.slice(1)[0];
